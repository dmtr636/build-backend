...
public class ProjectWorkCommentService extends BaseService<ProjectWorkComment, ProjectWorkCommentDTO> {
    ...
    public ProjectWorkComment create(ProjectWorkCommentDTO dto) throws ApiException {
        ...
        updateFiles(comment, dto.getFileIds());
    ...
    private void updateFiles(ProjectWorkComment comment, List<UUID> fileIds) {
        if (fileIds != null) {
            comment.setFileIds(new ArrayList<>(fileIds));
        } else {
            comment.getFileIds().clear();
        }
    }
